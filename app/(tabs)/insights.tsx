...
export default function ProgressScreen() {
  ...
  const getWeeklyAverage = (key: "calories" | "protein" | "carbs" | "fats") => {
    ...
    const total = weeklyData.reduce(
      (
        sum: number,
        day: { [key in "calories" | "protein" | "carbs" | "fats"]: number },
      ) => sum + (day[key] || 0),
      0,
    );
    ...
    >
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <Text
            style={[styles.title, { color: colors.text, textAlign: "left" }]}
          >
            {t("progress")}
          </Text>
          <Text
        ...
        >
          <View
            style={[
              styles.streakCardContent,
              isRTL && styles.streakCardContentRTL,
            ]}
          >
        ...
        >
          ...
          <View style={[styles.statCard, { backgroundColor: colors.surface }]}>
            ...
            <Text style={[styles.statValue, { color: colors.text }]}>
              {progressData.totalDaysLogged ?? 0}
            </Text>
            <Text
              style={[
                styles.statLabel,
        ...
        <View style={[styles.chartCard, { backgroundColor: colors.surface }]}>
          ...
          <View style={styles.chart}>
            <View style={[styles.chartBars, isRTL && styles.chartBarsRTL]}>
              {(isRTL ? [...weeklyData].reverse() : weeklyData).map(
                (day, i) => (
                  <View key={day.date || i} style={styles.barContainer}>
                    ...
                    <Text
                      style={[styles.barLabel, { color: colors.placeholder }]}
                    >
