...
export default function JournalScreen() {
  ...
  const getMealCalories = (mealType: "breakfast" | "lunch" | "dinner") => {
    return getMealFoods(mealType).reduce(
      (total: number, food: any) =>
        total + food.foodItem.calories * food.quantity,
      0,
    );
  };
          ...
          <View style={[styles.mealTitleContainer, isRTL && styles.rtlRow]}>
            ...
            <View style={[styles.mealInfo]}>
              ...
              {hasFood ? (
                ...
                >
                  {Math.round(mealCalories)} {t("calories")} •{" "}
                  {mealFoods.length}{" "}
                  {mealFoods.length > 1 ? t("items") : t("item")}
                </Text>
              ) : (
                <Text
                  style={[
